...
create Trigger insertbefore BEFORE INSERT ON users
...
create Function login(email VARCHAR(255), pwd VARCHAR(255))
RETURNS INT DETERMINISTIC
begin
DECLARE ok int;
set ok = 0;
SELECT id INTO ok FROM users WHERE email = users.email and password = pwd_encrypt(users.PASSWORD);
RETURN ok;
END
